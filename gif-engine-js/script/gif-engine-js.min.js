    ...
    try {
      ...
      while(++index < frameCount) {
        ...
        switch(disposals[index]) {
         case 2:
          for (let row = 0, offset; offset = (offsetLeft + (row + offsetTop) * canvasWidth) * 4, frameHeight > row; ++row)
         ...
         case 3:
          if (index > 0) {
            ...
            for (let row = 0, pointer = -1, offset; offset = (offsetLeft + (row + offsetTop) * canvasWidth) * 4, frameHeight > row; ++row)
            for (let column = 0; frameWidth > column; ++column) {
