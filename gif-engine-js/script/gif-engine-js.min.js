    ...
    try {
      ...
      while(++index < frameCount) {
        ...
        switch(disposals[index]) {
         case 2:
          for (let row = 0, column = 0, offset; offset = (column + offsetLeft + (row + offsetTop) * canvasWidth) * 4, frameHeight > row; column = 0, ++row)
         ...
         case 3:
          if (index > 0) {
            ...
            for (let row = 0, column = 0, pointer = -1, offset; offset = (column + offsetLeft + (row + offsetTop) * canvasWidth) * 4, frameHeight > row; column = 0, ++row)
            for (; frameWidth > column; ++column) {
